...
> 🚀 **v0.5.3** — ownership fix: `cgx analyze` was minting one bogus `Author` node per file (named after the file path), which polluted the graph with ~one singleton community per file. Authorship is now keyed by contributor, so `cgx docs` ownership is correct and the Louvain community count drops accordingly. Docs-vault polish: the Glossary collapses singleton communities and drops zero-count node kinds, undocumented module notes lose the empty "Description" column, `CrossClusterDeps` no longer emits dangling wiki-links, and `Owners.md` shows a real contributor → files-owned table. `cgx update --auto` now verifies the version actually changed instead of always reporting success. [v0.5.3 →](https://github.com/AayushBahukhandi/cgx/releases/tag/v0.5.3)
>
...
> **On v0.5.2?** Run `cgx update --auto` or reinstall. v0.5.3 fixes a graph-ingestion bug where `cgx analyze` created one bogus `Author` node per file (named after the path), inflating the community count with per-file singletons and producing a meaningless ownership view. **Re-run `cgx analyze --force` after upgrading** so authorship is re-keyed by contributor — you'll see the community count drop and `cgx docs` ownership become correct. Also polishes the docs vault (Glossary, module-note structure tables, cross-cluster links, owners table) and makes `cgx update --auto` verify the upgrade actually changed the version. No schema changes.
>
