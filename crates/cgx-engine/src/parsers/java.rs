...
fn extract_nodes(
    ...
    for m in cursor.matches(query, root, source_bytes) {
        ...
        let doc_comment = item_node
            .and_then(|n| collect_doc_block_above(n, source_bytes, is_jdoc_comment))
            .map(strip_jdoc_markers);
...
/// Javadoc / PHPDoc: comments starting with `/**`. Plain `//` line comments are noise.
fn is_jdoc_comment(text: &str) -> bool {
    text.trim_start().starts_with("/**")
}

fn strip_jdoc_markers(raw: String) -> String {
