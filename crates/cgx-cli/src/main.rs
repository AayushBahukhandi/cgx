...
enum Commands {
    ...
    Doctor {},
    /// Evaluate `.cgx/bisect.toml` predicates against the current graph and exit
    /// with `0` (good), `1` (bad), or `125` (skip) — suitable for `git bisect run`.
    BisectScript {
        /// Path to the repository
        #[arg(long)]
        repo: Option<PathBuf>,

        /// Custom predicate file (defaults to `.cgx/bisect.toml`).
        #[arg(long, value_name = "FILE")]
        config: Option<PathBuf>,

        /// Re-run `cgx analyze` before evaluating predicates.
        #[arg(long)]
        analyze: bool,

        /// Externally-supplied rule-violation count (from `cgx rules check`).
        #[arg(long, value_name = "N")]
        rule_violations: Option<u64>,

        /// Print the example `.cgx/bisect.toml` and exit.
        #[arg(long)]
        example: bool,
    },
...
enum DocsCmd {
    ...
    /// Generate a layered, Obsidian-ready docs vault from the graph
    Generate {
        /// Path to the repository
        #[arg(long)]
        repo: Option<PathBuf>,

        /// Write to a local folder (default: ./cgx-docs). Mutually exclusive with --vault / --vault-at.
        #[arg(long, conflicts_with_all = ["vault", "vault_at"])]
        local: bool,

        /// Output directory when using --local (defaults to ./cgx-docs, or [docs].output_dir from config).
        #[arg(long, value_name = "DIR")]
        out: Option<PathBuf>,

        /// Write to an Obsidian vault. Reads [docs].vault_path from .cgx/config.toml; if unset,
        /// auto-detects the most recently opened vault via Obsidian's config file.
        /// Use --vault-at for an explicit path that takes precedence.
        #[arg(long, conflicts_with = "vault_at")]
        vault: bool,

        /// Write to a specific Obsidian vault path. Overrides [docs].vault_path and auto-detect.
        #[arg(long = "vault-at", value_name = "PATH", conflicts_with = "vault")]
        vault_at: Option<PathBuf>,

        /// Only regenerate module notes whose graph slice changed since last run
        #[arg(long, conflicts_with = "force")]
        incremental: bool,

        /// Force a full rebuild of every note (default behaviour, but explicit).
        #[arg(long, conflicts_with = "incremental")]
        force: bool,
    },
    /// Show which module notes would be regenerated by `docs generate --incremental`
    Status {
        /// Path to the repository
        #[arg(long)]
        repo: Option<PathBuf>,
    },
    /// List or print AI prose stubs (`<!-- cgx-prompt -->` blocks) waiting to be filled
    Prompts {
        /// Path to the repository
        #[arg(long)]
        repo: Option<PathBuf>,

        /// Vault root to scan. If omitted, looks for ./cgx-docs/ then [docs].output_dir.
        #[arg(long, value_name = "DIR")]
        vault: Option<PathBuf>,

        /// Print just the next unfilled packet (for piping into an AI). Implies plain output.
        #[arg(long)]
        next: bool,

        /// Print one specific packet by source file path (e.g. crates/.../graph.rs).
    ...
    let result = match cli.command {
        ...
        Commands::Docs { cmd } => match cmd {
            ...
            DocsCmd::Generate {
                ...
                let repo_path = repo.unwrap_or_else(|| PathBuf::from("."));
                cmd_docs_generate(&repo_path, local, out, vault, vault_at, incremental, force)
            }
            DocsCmd::Status { repo } => {
                let repo_path = repo.unwrap_or_else(|| PathBuf::from("."));
                cmd_docs_status(&repo_path)
            }
            DocsCmd::Prompts {
                repo,
                vault,
                ...
                let repo_path = repo.unwrap_or_else(|| PathBuf::from("."));
                cmd_docs_prompts(&repo_path, vault, next, path)
            }
        },
        Commands::Complexity {
            repo,
        ...
        Commands::BisectScript {
            ...
            let repo_path = repo.unwrap_or_else(|| PathBuf::from("."));
            cmd_bisect_script(&repo_path, config, analyze, rule_violations, example)
        }
...
fn cmd_docs_generate(
    ...
    use cgx_engine::config::CgxConfig;
    use cgx_engine::docs::{generate_vault, DocsMode, DocsOptions, DocsTarget, WikiLinkStyle};
    let _ = force; // accepted as an explicit "full rebuild" alias; full is also the default.

    let canonical = repo_path
        .canonicalize()
        .unwrap_or_else(|_| repo_path.to_path_buf());
    let db = GraphDb::open(&canonical)?;
    let config = CgxConfig::load(&canonical).unwrap_or_default();
    let docs_cfg = config.docs;

    // Resolve target. `--vault` = auto-detect, `--vault-at` = explicit path,
    // otherwise default to a local folder. `--local` is accepted for explicitness
    // but behaves the same as the default branch.
    let _ = local;
    let target = if vault {
        // Honour [docs].vault_path from config if set, otherwise auto-detect.
        if !docs_cfg.vault_path.is_empty() {
            DocsTarget::Vault(Some(PathBuf::from(&docs_cfg.vault_path)))
        } else {
            DocsTarget::Vault(None)
        }
    } else if let Some(path) = vault_at {
        DocsTarget::Vault(Some(path))
    } else {
        let out_dir = out
            .or_else(|| {
                if !docs_cfg.output_dir.is_empty() {
                    Some(PathBuf::from(&docs_cfg.output_dir))
                } else {
                    None
                }
            })
            .unwrap_or_else(|| PathBuf::from("./cgx-docs"));
        DocsTarget::Local(out_dir)
    };
    ...
    let opts = DocsOptions {
        prompt_packets: docs_cfg.prompt_packets,
        frontmatter: docs_cfg.frontmatter,
        wiki_links_style: if docs_cfg.wiki_links == "markdown" {
            WikiLinkStyle::Markdown
        } else {
            WikiLinkStyle::Obsidian
        },
        include_dead_code: docs_cfg.include_dead_code,
        include_duplicates: docs_cfg.include_duplicates,
    };

    let report = generate_vault(&canonical, &db, target, mode, &opts)?;

    println!();
    println!("  cgx docs ({})", report.mode);
    println!("  {}", "\u{2500}".repeat(60));
    println!("  Output:           {}", report.output_dir.display());
    println!(
        "  Module notes:     {} written, {} skipped",
        report.module_notes_written, report.module_notes_skipped
    );
    println!("  Index notes:      {} written", report.index_notes_written);
    println!();
    println!(
        "  Open `{}` in Obsidian (or any markdown viewer) to browse.",
...
fn cmd_docs_prompts(
    ...
    use cgx_engine::config::CgxConfig;
    use cgx_engine::docs::prompts_index::list_unfilled;
    ...
    // Resolve which vault root to scan.
    let vault_root = vault
        .or_else(|| {
            let local = canonical.join("cgx-docs");
            if local.is_dir() {
                Some(local)
            } else {
                None
        ...
        .or_else(|| {
            let cfg = PathBuf::from(&config.docs.output_dir);
            if cfg.is_dir() {
                Some(cfg)
            } else {
                None
    ...
    let Some(root) = vault_root else {
        anyhow::bail!(
            "No vault found. Generate one first with `cgx docs generate` or pass `--vault <path>`."
        );
    };

    let entries = list_unfilled(&root)?;

    if let Some(pf) = path_filter {
        for e in &entries {
            if e.source_path.as_deref() == Some(pf.as_str()) {
                println!("{}", e.packet);
                return Ok(());
            }
        }
        anyhow::bail!("No unfilled packet found for source path: {}", pf);
    }

    if next {
        if let Some(e) = entries.first() {
            println!("# Note: {}", e.note_path.display());
            if let Some(p) = &e.source_path {
                println!("# Source: {}", p);
            }
            println!();
            println!("{}", e.packet);
        } else {
            println!("(no unfilled packets)");
        }
        return Ok(());
    }

    if entries.is_empty() {
        println!("All {} module note(s) are filled.", count_notes(&root));
        return Ok(());
    }

    println!();
    println!("  Unfilled AI prose stubs ({} total)", entries.len());
    println!("  {}", "\u{2500}".repeat(60));
    for e in &entries {
        let display_path = e
            .note_path
            .strip_prefix(&root)
            .unwrap_or(&e.note_path)
            .display();
        if let Some(p) = &e.source_path {
    ...
    println!();
    println!("  Tip: `cgx docs prompts --next` prints the next packet (pipe into your AI),");
    println!("       `cgx docs prompts --path <src>` prints a specific one.");
    println!();
    Ok(())
}
...
fn count_notes(root: &Path) -> usize {
    let mut count = 0;
    if let Ok(entries) = std::fs::read_dir(root.join("30-Modules")) {
        for e in entries.flatten() {
            if e.path().is_dir() {
                if let Ok(inner) = std::fs::read_dir(e.path()) {
                    count += inner
                        .flatten()
                        .filter(|f| f.path().extension().and_then(|s| s.to_str()) == Some("md"))
                        .count();
                }
            }
        }
...
fn cmd_docs_status(repo_path: &Path) -> anyhow::Result<()> {
    use cgx_engine::docs::incremental::{self, slice_hash};

    let canonical = repo_path
        .canonicalize()
        .unwrap_or_else(|_| repo_path.to_path_buf());
    let db = GraphDb::open(&canonical)?;
    let state = incremental::load_state(&db.repo_id);
    ...
    println!();
    println!("  cgx docs status");
    println!("  {}", "\u{2500}".repeat(60));
    if state.generated_at.is_empty() {
        println!("  Last generation:  never");
    } else {
        println!("  Last generation:  {}", state.generated_at);
    }
    println!();
    for p in &file_paths {
        let Ok(summary) = db.get_file_summary(p) else {
            continue;
        };
        let h = slice_hash(&summary);
        match state.files.get(p) {
            Some(entry) if entry.slice_hash == h => unchanged += 1,
            Some(_) => {
                changed += 1;
                println!("  ~ {}", p);
            }
...
fn cmd_bisect_script(
    ...
    use cgx_engine::bisect::{BisectPredicates, EXAMPLE_TOML};
    ...
    if analyze {
        // Run an incremental re-index before evaluating — keeps the bisect loop
        // fast on commits where little changed, and accurate on commits where
        // a lot did.
        let db = GraphDb::open(&canonical)?;
        let _ = cgx_engine::analyze_repo_incremental(&canonical, &db, true, false, false, false)?;
    }

    let predicates = match BisectPredicates::load(&canonical, config.as_deref()) {
        Ok(p) => p,
        Err(e) => {
            eprintln!("cgx bisect-script: {}", e);
    ...
    if let Some(actual) = rule_violations {
        if let Some(max) = predicates.rule_violations_max {
            ...
            for o in &mut report.outcomes {
                if o.predicate == "rule_violations_max" {
                    o.passed = passed;
                    o.detail = format!("required ≤ {}, got {}", max, actual);
                }
            }
        }
    }

    let verdict = if report.passed() { "GOOD" } else { "BAD" };
    println!(
        "cgx bisect-script: {} ({} predicate(s))",
        verdict,
        report.outcomes.len()
    );
    for o in &report.outcomes {
        let mark = if o.passed { "✓" } else { "✗" };
        println!("  {} {} — {}", mark, o.predicate, o.detail);
    }
